...
available in the `archived onix-commutator git repo	<https://github.com/open-ephys/onix-commutator>`_. 
...
..  note:: 
		...
		firmware. This can be done using the `picotool config` command if you have a newer commutator 
		with USB-C or flashing the firmware if you have an older commutator with Micro-USB. 
...
electrically conductive connections. The commutator front panel is a PCB. Fortunately, PCBs are
pretty cheap to fabricate. The fab-house usually requires that you upload `Gerber files
